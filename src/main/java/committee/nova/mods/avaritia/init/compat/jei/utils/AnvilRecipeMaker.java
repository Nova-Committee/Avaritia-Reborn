package committee.nova.mods.avaritia.init.compat.jei.utils;
...
public class AnvilRecipeMaker {
    ...
    private static final class EnchantmentData {
        ...
        public List<ItemStack> getEnchantedBooks(ItemStack ingredient) {
            return enchantedBooks.stream()
                    .filter(enchantedBook -> ingredient.getItem().isBookEnchantable(ingredient, enchantedBook))
                    .toList();
        }
        ...
        private boolean canEnchant(ItemStack ingredient) {
            try {
                ...
                LOGGER.error("Failed to check if ingredient can be enchanted:", e);
                return false;
            }
        }
    ...
    private static Stream<IJeiAnvilRecipe> getBookEnchantmentRecipes(
        ...
        List<EnchantmentData> enchantmentDatas = ForgeRegistries.ENCHANTMENTS.getValues()
                .stream()
    ...
    public static int findLevelsCost(ItemStack leftStack, ItemStack rightStack) {
        ...
        try {
            ...
            Static.LOGGER.error("Could not get anvil level cost for.", e);
            return -1;
        }
    }
}
